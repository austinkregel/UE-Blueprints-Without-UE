<template>
  <div class="flex flex-wrap items-center gap-2 overflow-x-auto px-4 py-2">
    ...
    <div class="ml-auto flex items-center gap-3 text-xs text-zinc-600 dark:text-zinc-300">
      ...
      <span v-if="debugMode">X: {{ Math.round(viewport.x) }}, Y: {{
          Math.round(viewport.y)
        }}, Zoom: {{ viewport.zoom.toFixed(2) }}</span>
      <span v-if="executionSummary?.isComplete || isExecuting" class="text-xs">
                <span v-if="executionSummary?.isComplete && executionSummary?.errors === 0"
                      class="text-green-600 dark:text-green-400"
                >✅ {{ executionSummary.executedNodes }}/{{ executionSummary.totalNodes }}</span
                >
                <span v-else-if="executionSummary?.isComplete && executionSummary?.errors > 0"
                      class="text-yellow-600 dark:text-yellow-400"
                >⚠️ {{ executionSummary.executedNodes }}/{{ executionSummary.totalNodes }} ({{
                    executionSummary.errors
                  }} errors)</span
                >
                <span v-else-if="isExecuting"
                      class="text-blue-600 dark:text-blue-400">🔄 {{ executionSummary?.executedNodes }} done</span>
            </span>
    </div>
  </div>
</template>
