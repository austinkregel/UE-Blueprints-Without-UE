<template>
    ...
    >
        ...
        <div v-if="needsSpecialContent" class="p-2 text-xs text-gray-400">
            <div v-if="node.nodeDefId === 'sequence'">Executes outputs in order</div>
            <div v-else-if="node.nodeDefId === 'gate'">State: {{ gateState }}</div>
            <div v-else-if="node.nodeDefId === 'multigate'">Next: {{ nextOutput }}</div>
            ...
            <div v-else-if="node.nodeDefId === 'delay'">Duration: {{ node.inputs?.find((i) => i.name === 'Duration')?.defaultValue || 1.0 }}s</div>
            <div v-else-if="node.nodeDefId === 'flip_flop'">Next: {{ isFlipState ? 'A' : 'B' }}</div>
        </div>
    </NodeBase>
</template>
...
<script setup>
    ...
    const needsSpecialContent = computed(() => {
        const specialNodes = ['sequence', 'gate', 'multigate', 'do_once', 'do_n', 'delay', 'retriggerable_delay', 'flip_flop'];
