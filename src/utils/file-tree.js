...
export async function listDirectory(path) {
  ...
  try {
    ...
    // Map to UI shape
    return (entries || []).map(e => ({ name: e.name, path: e.path, kind: e.is_dir ? 'dir' : 'file', children: e.is_dir ? [] : undefined }));
  } catch (e) {
...
export async function readDirectoryTree(root, { includeHidden = false } = {}) {
  if (!isTauriAvailable()) return { name: root?.split('/')?.pop() || 'root', path: root, kind: 'dir', children: [], warnings: ['Not running in Tauri'] };
...
export async function readText(path) {
  ...
  const invoke = await getInvoke();
  if (invoke) {
    try { return await invoke('read_text_file', { path }); } catch (e) { console.error('read_text_file failed', e); return ''; }
  }
