...
function emitExecutionEvent(evt) {
  ...
  _listeners.forEach(fn => { try { fn(withTime); } catch (_) {} });
}

async function executionGate(node) {
  const hitBp = breakpoints.value.has(node.id);
  if (stepMode.value || isPaused.value || hitBp) {
    emitExecutionEvent({ type: 'paused', nodeId: node.id, reason: hitBp ? 'breakpoint' : (isPaused.value ? 'paused' : 'step') });
    await new Promise(resolve => { _gateResolve = resolve; });
  }
}

export function setStepMode(enabled) { stepMode.value = !!enabled; }
export function pauseInteractive() { isPaused.value = true; }
export function resumeInteractive() { isPaused.value = false; if (_gateResolve) { const r = _gateResolve; _gateResolve = null; r(); } }
export function stepOnce() { if (_gateResolve) { const r = _gateResolve; _gateResolve = null; r(); } }
export function toggleBreakpoint(nodeId) { if (breakpoints.value.has(nodeId)) breakpoints.value.delete(nodeId); else breakpoints.value.add(nodeId); }
export function clearBreakpoints() { breakpoints.value.clear(); }
...
async function executeNextNodes(nodeId, outputName = null) {
  ...
  for (const c of connections) {
    emitExecutionEvent({ type: 'edge-traverse', from: { nodeId: c.from.nodeId, output: c.from.output }, to: { nodeId: c.to.nodeId, input: c.to.input } });
  }
  ...
  try {
    ...
    if (entryPoints.length === 0) {
      ...
      const startNodes = nodes.value.filter(node => 
        node.outputs?.some(output => output.type === 'exec')
      );
    ...
    console.log(`Found ${entryPoints.length} entry points:`, entryPoints.map(n => n.nodeDefId || n.type));
...
export function stopExecution() {
  ...
  if (_gateResolve) { const r = _gateResolve; _gateResolve = null; r(); }
...
export function clearInputOverrides() { inputOverrides.value.clear(); }

// === Validation & Evaluation API ===
function isExecType(t) { return String(t || '').toLowerCase() === 'exec'; }
...
function normalizeSinks(sinks) {
  ...
  return sinks.map(s => {
    if (typeof s === 'string') { const [nid, out] = s.split('.'); return { nodeId: Number(nid), output: out }; }
    return s;
  });
}
...
export function validateGraphInputs({ sinks = null, overrides = null } = {}) {
  ...
  const ov = new Map();
  if (overrides instanceof Map) { overrides.forEach((v,k)=>ov.set(String(k), v)); }
  ...
  neededInputs.forEach(key => {
    ...
    if (!incoming.length) {
      ...
      if (inType !== 'mixed' && outType !== 'mixed' && inType !== outType) {
        warns.push({ from: { nodeId: incoming[0].from.nodeId, output: incoming[0].from.output, type: outType }, to: { nodeId: nid, input: iname, type: inType }, reason: 'type_mismatch' });
      }
    }
  });
...
async function evaluatePureDataflow({ overrides } = {}) {
  ...
  while (progress && guard < nodes.value.length) {
    ...
    for (const node of nodes.value) {
      ...
      for (const inp of (node.inputs || [])) {
        ...
        if (!incoming.length) {
          if (inp.defaultValue === undefined) { ready = false; break; }
        } else {
          const srcId = incoming[0].from.nodeId; const srcOut = incoming[0].from.output;
          const srcRes = executionResults.value.get(srcId);
          if (!srcRes || !(srcOut in srcRes)) { ready = false; break; }
        }
      }
      if (!ready) continue;
      // Evaluate this node
...
export async function evaluateGraphToSinks({ entryPoints = [], overrides = null, sinks = null } = {}) {
  ...
  await evaluatePureDataflow({ overrides });
