...
export function onConnectionDragEnd(e) {
  ...
  if (nearestIO) {
    ...
    if ((drag.type === 'output' || drag.type === 'exec') && nearestIO.side === 'inputs') {
      connectNodes({ from: { nodeId: drag.from.nodeId, output: drag.from.output }, to: { nodeId: nearestIO.nodeId, input: nearestIO.ioName } });
      draggingConnection.value = null;
      return;
    } else if (drag.type === 'input' && nearestIO.side === 'outputs') {
      connectNodes({ from: { nodeId: nearestIO.nodeId, output: nearestIO.ioName }, to: { nodeId: drag.to.nodeId, input: drag.to.input } });
      draggingConnection.value = null;
      return;
    }
  }
