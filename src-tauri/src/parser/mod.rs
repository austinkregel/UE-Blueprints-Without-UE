...
pub struct SymbolItem {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub visibility: Option<String>, // public|protected|private|export|pub|package
    #[serde(skip_serializing_if = "Option::is_none")]
    pub is_entry_point: Option<bool>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub is_method: Option<bool>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner_type: Option<String>, // class/struct/trait name if method
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct ReferenceItem {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub qualifier: Option<String>, // object/type qualifier text if present
    #[serde(skip_serializing_if = "Option::is_none")]
    pub range: Option<Range>,
}
...
impl Registry {
    ...
    pub fn get(&self, id: &str) -> Option<&dyn LanguageAdapter> {
...
pub fn parse_with_adapter(
    ...
    if nf.file_path.is_empty() {
        nf.file_path = file_path.to_string();
    }
    if nf.language.is_empty() {
        nf.language = adapter.id().to_string();
    }
    Ok(nf)
}
...
mod tests {
    ...
    "#;
        ...
        let f = nf
            .symbols
            .iter()
            .find(|s| s.kind == "function" && s.name == "foo")
            .expect("foo found");
    ...
    "#;
        ...
        let m = nf
            .symbols
            .iter()
            .find(|s| s.kind == "function" && s.name == "m")
            .expect("m found");
        ...
        let f = nf
            .symbols
            .iter()
            .find(|s| s.kind == "function" && s.name == "f")
            .expect("f found");
        assert_eq!(f.is_method, Some(false));
        assert_eq!(f.is_entry_point, Some(true));
    }
...
"#;
        ...
        let foo = nf
            .symbols
            .iter()
            .find(|s| s.kind == "class" && s.name == "Foo")
            .expect("Foo class found");
