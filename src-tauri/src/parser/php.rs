        ...
        if k.contains("namespace") {
          ...
          if !txt.contains('{') {
            ...
            let mut ns_name = if let Some(nm_node) = ch.child_by_field_name("name") {
              nm_node.utf8_text(source.as_bytes()).unwrap_or("").to_string()
            } else {
              // Fallback: look for a child whose kind ends with "name"
              let mut j = 0; let mut name: Option<String> = None;
              while j < ch.child_count() {
                if let Some(nn) = ch.child(j) {
                  let ck = nn.kind();
                  if ck.ends_with("name") {
                    name = Some(nn.utf8_text(source.as_bytes()).unwrap_or("").to_string());
                    break;
                  }
                }
                j += 1;
            ...
            while ns_name.contains("\\\\") { ns_name = ns_name.replace("\\\\", "\\"); }
            ns_name = ns_name.trim_matches('\\').to_string();
            if !ns_name.is_empty() { default_ns = Some(ns_name); }
          }
      ...
      for s in nf.symbols.iter_mut() {
        match s.kind.as_str() {
          "class" | "interface" | "trait" | "enum" | "function" | "type_alias" | "union" => {
            ...
            if should_patch { s.fqn = Some(format!("{}\\{}", ns, s.name)); }
          }
          _ => {}
        }
      }
