use super::{to_range, NormalizedFile, ParamItem, ReferenceItem, SymbolItem};
...
fn has_field(node: tree_sitter::Node, field: &str) -> bool {
...
fn is_type_like(kind: &str) -> Option<&'static str> {
    ...
    if k.contains("type_alias") || k.contains("type-alias") || k == "type_declaration" {
...
fn is_const_like(kind: &str) -> bool {
    let k = kind.to_lowercase();
    k.contains("const")
        || k.contains("constant_declaration")
        || k.contains("constant-declaration")
        || k == "constant"
        || k.contains("static")
}
...
fn is_function_like(node: tree_sitter::Node) -> bool {
    ...
    let has_params = has_field(node, "parameters")
        || has_field(node, "parameter_list")
        || has_field(node, "formal_parameters")
        || has_field(node, "params");
...
fn node_name(src: &str, node: tree_sitter::Node) -> Option<String> {
    ...
    while i < node.child_count() {
        if let Some(ch) = node.child(i) {
            let ck = ch.kind();
            if ck.ends_with("identifier") || ck == "name" || ck.ends_with("name") {
                return Some(text_of(src, ch));
            }
        }
        i += 1;
    }
    ...
    if let Some(list) = param_container {
        ...
        while i < list.named_child_count() {
            if let Some(pn) = list.named_child(i) {
                ...
                let looks_like_param = pk.contains("param")
                    || pk.contains("parameter")
                    || pk.contains("argument")
                    || pk.contains("typed_pattern")
                    || pk.contains("required_parameter");
                if looks_like_param {
...
fn extract_return_type(src: &str, func_node: tree_sitter::Node) -> Option<String> {
    for f in [
        ...
        if let Some(n) = func_node.child_by_field_name(f) {
            return Some(text_of(src, n));
        }
    }
    if let Some(params) = func_node
        .child_by_field_name("parameters")
        .or_else(|| func_node.child_by_field_name("parameter_list"))
        .or_else(|| func_node.child_by_field_name("formal_parameters"))
        .or_else(|| func_node.child_by_field_name("params"))
    {
        ...
        let mut i = 0;
        while i < func_node.named_child_count() {
            if let Some(ch) = func_node.named_child(i) {
                if ch.start_byte() >= params_end && ch.kind().to_lowercase().contains("type") {
                    return Some(text_of(src, ch));
                }
            }
            i += 1;
...
fn has_export_ancestor(node: tree_sitter::Node) -> bool {
    ...
    for _ in 0..3 {
        if let Some(p) = cur.parent() {
            let k = p.kind();
            if k == "export_statement" || k == "export_declaration" {
                return true;
            }
            cur = p;
        } else {
...
fn detect_visibility(src: &str, node: tree_sitter::Node) -> (Option<String>, Option<bool>) {
    ...
    while i < node.child_count() {
        if let Some(ch) = node.child(i) {
            ...
            if k == "pub" || k.contains("public") {
                return (Some("public".to_string()), Some(true));
            }
            if k.contains("export") {
                return (Some("export".to_string()), Some(true));
            }
            if k.contains("protected") {
                return (Some("protected".to_string()), Some(false));
            }
            if k.contains("private") {
                return (Some("private".to_string()), Some(false));
            }
            if k.contains("visibility") || k.contains("modifier") {
                let t = text_of(src, ch);
                let tl = t.to_lowercase();
                if tl.contains("public") || tl.contains("pub") {
                    return (Some("public".to_string()), Some(true));
                }
                if tl.contains("protected") {
                    return (Some("protected".to_string()), Some(false));
                }
                if tl.contains("private") {
                    return (Some("private".to_string()), Some(false));
                }
                if tl.contains("export") {
                    return (Some("export".to_string()), Some(true));
                }
...
fn has_type_like_ancestor(mut node: tree_sitter::Node, src: &str) -> Option<String> {
    ...
    while depth < 3 {
        if let Some(p) = node.parent() {
            if is_type_like(p.kind()).is_some() {
...
pub fn normalize_generic(
    ...
    fn walk(
        nf: &mut NormalizedFile,
        src: &str,
        node: tree_sitter::Node,
        ns_stack: &mut Vec<String>,
        type_owner_stack: &mut Vec<String>,
    ) {
        let kind = node.kind();
        ...
        if let Some(mapped) = is_type_like(kind) {
            if let Some(nm) = node_name(src, node) {
                let fqn = if ns_stack.is_empty() {
                    nm.clone()
                } else {
                    format!("{}::{}", ns_stack.join("::"), nm)
                };
        ...
        if is_function_like(node) {
            if let Some(nm) = node_name(src, node) {
                ...
                let owner =
                    has_type_like_ancestor(node, src).or_else(|| type_owner_stack.last().cloned());
                let params = extract_params(src, node);
                let ret_ty = extract_return_type(src, node);
                let mut fqn = None;
                if let Some(owner_name) = owner.clone() {
                    fqn = Some(if ns_stack.is_empty() {
                        format!("{}::{}", owner_name, nm)
                    } else {
                        format!("{}::{}::{}", ns_stack.join("::"), owner_name, nm)
                    });
        ...
        if is_call_like(&lk) {
            ...
            if let Some(n) = name {
                ...
                nf.references.as_mut().unwrap().push(ReferenceItem {
                    kind: "call".to_string(),
                    name: nm,
                    qualifier,
                    range: Some(to_range(node)),
        ...
        while i < node.child_count() {
            if let Some(ch) = node.child(i) {
                walk(nf, src, ch, ns_stack, type_owner_stack);
            }
            i += 1;
        }
